...
def get_pose_for_folder(global_config):
    ...
    for name in tqdm(os.listdir(target_folder + '/poses_R')):
        # for rand_ind in range(0,100):
            ...
            xs,ys,zs = get_T_limits(f,[w,h],sensor_width,pose_and_shape_pose,bbox,gt_z)
            Ts = init_Ts(xs,ys,zs)
            ...
            distances_to_gt[detection] = distance

            continue

    with open(target_folder + '/global_stats/closest_dist_to_gt_3_0_90_steps.json','w') as f:
        json.dump(distances_to_gt,f)
            ...
            # T = Ts[best_T_index]
            # n_indices = 1
            # pose_information = create_pose_info_dict(np.array(R),T,n_indices,max_factor,gt_pose_in_limits,gt_rot,gt_trans,best_R_possible,best_T_possible,xs,ys,zs,tilts,elevs,azims,scaling)
            ...
            #     elif signal == 'points':
            #         if points_available:
            #             img = plot_points_T(torch.Tensor(R).unsqueeze(0),torch.Tensor(T).unsqueeze(0).unsqueeze(0),scaling,model_path,img_path,sw,device,points_3D,points_2D,B,f,top_n_for_translation,multiplier_points,point_threshold,use_threshold)
            #         else:
            #             img = cv2.imread(target_folder + '/images/' + gt_infos["img"])
