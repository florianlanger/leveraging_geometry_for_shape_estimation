...
def one_epoch_classifier(dataset,net,optimizer,criterion,epoch,writer,kind,device,config,exp_path):
    ...
    # for num_batch in range(N_total_batches):
        ...
        for iter_refinement in range(N_refinements):
            ...
            # inputs = torch.zeros((config['training']["batch_size"],30000,10),device=device)
            # targets = torch.zeros((config['training']["batch_size"],11),device=device)
...
def create_data_loaders(config):
    ...
    if config["data"]["type"] == 'points' and config["data"]["what_models"] == "lines":
        ...
        # from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset_points_v2 import Dataset_points
        from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset_points_v3_debug_multi_gpu import Dataset_points
...
def main():
    ...
    device_ids = list(range(len(config["general"]["gpu"].split(','))))
