...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.batch_sampler_v4 import get_batch_from_dataset,get_index_infos,get_simple_batch_from_dataset
...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset_points_v4 import Dataset_points
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.visualisation.visualisation_main_v4 import visualise_preds
...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.losses.loss_v4 import get_combined_loss
    ...
    # for num_batch in range(N_total_batches):
        ...
        for iter_refinement in range(N_refinements):
            ...
            if config["loss"]["use_geometric_loss"] == False:
                loss,latest_metrics = get_combined_loss(outputs,targets,config,just_classifier=False)
            ...
            with torch.no_grad():
                ...
                if num_batch == 0 and epoch % config['training']["vis_interval"] == 0:
                    visualise_preds(writer,latest_metrics,inputs,net,config,kind,epoch,extra_infos,exp_path,num_batch,iter_refinement,dataset.transform_Rs,roca_eval_combo)


                index_infos = get_index_infos(outputs,extra_infos,config,iter_refinement,dataset)
                t6 = time.time()
...
def add_missing_keys_to_config(config):
    ...
    if 'roca_eval_interval' not in config['training']:
        config['training']['roca_eval_interval'] = 50

    if "N_rotations" not in config["data_augmentation"]:
        config["data_augmentation"]["N_rotations"] = 4
    default_false = ['use_all_points_2d',"use_random_points",'input_rgb',"input_RST_and_CAD_ID","add_history"]
...
def process_config(config):
    ...
    if config['model']["regress_offsets"] == False:
        ...
        config["model"]["n_outputs"] = 11 + config["data_augmentation"]["N_rotations"]
    ...
    if config["data"]["sample_what"] == 'T':
        ...
        if config["loss"]["train_only_classifier"] == False:
            config["loss"]["constants_multiplier"]["classification"] = 0.0
        else:
            config["loss"]["constants_multiplier"]["classification"] = 1.0
            config["loss"]["constants_multiplier"]["s"] = 0.0
            config["loss"]["constants_multiplier"]["r"] = 0.0
...
def evaluate_checkpoint(checkpoint_path,n_refinements,index_rotation,run_on_octopus=True,gpu_number=0):
    ...
    # print('DEBUG N rotations 8')
    # config["data_augmentation"]["N_rotations"] = 8
...
def track_gpu_usage(writer,step):
    ...
    gpu_stat_names = ['gpu_memory_allocated','gpu_memory_reserved','gpu_memory_allocated','gpu_max_memory_allocated','gpu_max_memory_reserved','gpu_max_memory_allocated']
    gpu_stat_values = [torch.cuda.memory_allocated(),torch.cuda.memory_reserved(),torch.cuda.memory_allocated(),torch.cuda.max_memory_allocated(),torch.cuda.max_memory_reserved(),torch.cuda.max_memory_allocated()]
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        network,optimizer,exp_path,config,device,start_epoch = evaluate_checkpoint(checkpoint_path,n_refinements,index_rotation,run_on_octopus=True,gpu_number=0)
    ...
    # checkpoint_path = '/scratches/octopus/fml35/experiments/regress_T/runs_10_T_and_R_and_S/date_2022_06_16_time_17_46_25_five_refinements_depth_aug_points_in_bbox_3d_coords/saved_models/network_epoch_000150.pth'
    ...
    if len(sys.argv) > 2:
        with torch.no_grad():
            validate_roca(val_roca_dataset_all_images,network,optimizer,criterion,config["training"]["roca_eval_interval"],writer,device,config,exp_path)
            # validate_roca(val_roca_dataset,network,optimizer,criterion,config["training"]["save_interval"],writer,device,config,exp_path)
...
if __name__ == "__main__":
    print('600 Points per model !!!')
    # print(' debug change load canny point function to not load them')
    # print('inititalise with cad id median')
