...
def load_rgb_image(config,kind,gt_name,normalised=True):
    ...
    path_img = dir_path_2d + '/images/' + gt_name.replace('.json','.jpg')
...
def load_gt_reprojection(config,gt_name):
    path = '/scratches/octopus_2/fml35/datasets/scannet/scannet_frames_25k_own_data_exp_12_calibration_matrix/' + gt_name.split('-')[0] + '/overlayed_instance_rerendered/' + gt_name.split('-')[1].split('.')[0] + '.png'
