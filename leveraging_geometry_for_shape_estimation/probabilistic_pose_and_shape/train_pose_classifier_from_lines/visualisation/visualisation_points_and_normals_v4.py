...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.visualisation.utils import load_rgb_image,draw_points_on_image,load_gt_reprojection,expand_indices
from leveraging_geometry_for_shape_estimation.utilities.write_on_images import draw_text_block



def plot_points_preds_normals(points, labels,probabilities,correct,t_pred,s_pred,r_pred,t_correct,s_correct,r_correct,config,extra_infos,kind,transform_Rs,no_render):
    ...
    for idx in np.arange(n_examples):
        ...
        text = ["gt:{},pred:{:.4f}".format(labels[idx].item(),np.max(probabilities[idx])),
        ...
        img_rgb = vis_render(extra_infos,idx,config,original_image.copy(),t_pred,s_pred,r_pred,probabilities,transform_Rs,no_render=no_render)
        draw_points_on_image(points[idx],img_rgb,size=1,which_channels=[3,2])
        images.append(img_rgb)
        img_rgb = vis_render(extra_infos,idx,config,original_image.copy(),t_pred,s_pred,r_pred,probabilities,transform_Rs,vis_offset=True,no_render=no_render)
        images.append(img_rgb)
...
def vis_render(extra_infos,idx,config,original_image,t_pred,s_pred,r_pred,probabilities,transform_Rs,vis_offset=False,no_render=False):
    if no_render == True:
        return original_image

    else:
        from leveraging_geometry_for_shape_estimation.vis_pose.vis_pose import load_mesh,render_mesh,overlay_rendered_image,render_mesh_from_calibration
        ...
        K_path = dir_path_calibrations + extra_infos["gt_name"][idx].split('-')[0] + '/intrinsics_color.txt'
        K = load_4by4_from_txt(K_path)
        width,height = extra_infos["img_size"][idx][0],extra_infos["img_size"][idx][1]
        ...
        if vis_offset == False:
            ...
            r_index = probabilities[idx]
            R = np.matmul(extra_infos['R'][idx],transform_Rs[-r_index])
            R = np.matmul(R,r_offset_pred)
            # R  = np.matmul(extra_infos['R_gt'][idx],np.matmul(np.linalg.inv(r_offset_pred),transform_Rs[r_index]))
        ...
        rendered_image = render_mesh_from_calibration(width,height,K,mesh,device)
...
def load_image(config,kind,gt_name,type='rgb'):
    ...
    if type == 'rgb' or type == 'normal':
        if type == 'rgb':
            ...
            path = dir_path_2d + '/norm_{}_{}/'.format(config['data']['img_size'][0],config['data']['img_size'][1]) + gt_name.replace('.json','.png')
