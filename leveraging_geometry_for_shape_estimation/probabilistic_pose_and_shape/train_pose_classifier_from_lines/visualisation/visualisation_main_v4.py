...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.visualisation.visualisation_images import plot_classes_preds
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.visualisation.visualisation_points import plot_points_preds
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.visualisation.visualisation_points_and_normals_v4 import plot_points_preds_normals
...
def visualise_preds(writer,latest_metrics,inputs,net,config,kind,epoch,extra_infos,exp_path,num_batch,iter_refinement,transform_Rs,roca_eval_combo=None):
    ...
    if config["data"]["type"] == 'lines':
        writer.add_figure('{} predictions'.format(kind),plot_classes_preds(inputs,labels,probabilities,correct,config,extra_infos,kind),epoch)
    elif config["data"]["type"] == 'points' and config['data']['dims_per_pixel'] == 3:
        writer.add_figure('{} predictions'.format(kind),plot_points_preds(inputs.cpu(), labels,probabilities,correct,config,extra_infos,kind),epoch)
    elif config["data"]["type"] == 'points' and (config['data']['dims_per_pixel'] == 7 or config['data']['dims_per_pixel'] == 10 or config['data']['dims_per_pixel'] == 13):
        no_render = not config["general"]["run_on_octopus"]
        images,info_vis_3d = plot_points_preds_normals(inputs.cpu(), labels,probabilities,correct,t_pred,s_pred,r_pred,t_correct,s_correct,r_correct,config,extra_infos,kind,transform_Rs,no_render=no_render)
        ...
        # plt.savefig(exp_path + '/vis/{}_predictions_{}.png'.format(kind,epoch),dpi=200)
        # writer.add_figure('{} predictions'.format(kind),fig,epoch)
...
def save_images(images,epoch,exp_path,kind,extra_infos,probabilities,correct,labels,num_batch,iter_refinement,roca_eval_combo=None):
    ...
    for i in range(len(images)):
        ...
        for j in range(len(images[i])):
            plt.imsave(base_path + '_{}_pred_{}_label_{}_{}.png'.format(names[j],np.round(probabilities[i].item(),3),np.round(labels[i].item(),3),str(correct[i].item())),images[i][j])
        combined = combine_images(images[i])
        combined = cv2.cvtColor(combined,cv2.COLOR_BGR2RGB)
        cv2.imwrite(base_path + '_combined_pred_{}_label_{}_{}.png'.format(np.round(probabilities[i].item(),3),np.round(labels[i].item(),3),str(correct[i].item())),combined)
