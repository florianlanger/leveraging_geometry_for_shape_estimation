...
def get_S(config,scaling_limits,scaling_medians,model_to_infos,model_path,gt_scaling_orig,category,sampled_S,kind,decrease_range_factor):
...
class Dataset_lines(data.Dataset):
    ...
    def get_R(self,R_gt,sampled_R,sym):
        if sampled_R is None:
            if self.kind == 'train' or self.kind == 'val':
                ...
                R,r_index = self.rotate_R_v2(R,sym)
                r_correct = False
    ...
    def get_R_for_regression(self,R_gt,sampled_R,sym):
        if sampled_R == None:

            random_angles = np.random.uniform(-np.array(self.config["data_augmentation"]["change_R_angle_degree"]),self.config["data_augmentation"]["change_R_angle_degree"],size=3)
            inv_offset_R = scipy_rot.from_euler('zyx',random_angles, degrees=True)
            R = np.matmul(R_gt,inv_offset_R.as_matrix())
            R,r_index = self.rotate_R_v3(R,sym)
            offset_R = inv_offset_R.inv().as_quat()
            ...
            compare_rots_classification = np.matmul(R_gt,self.transform_Rs)

            Rs = np.matmul(np.array(R).T,compare_rots_classification)
            angles = get_total_angle(Rs)
            index_closest = np.argmin(angles)
            r_index = index_closest % len(self.transform_Rs)

        
        if offset_R[3] < 0:
            offset_R = offset_R * -1
        return R,offset_R,r_index
    ...
    def rotate_R_v3(self,R,sym):
        n_rots = self.config["data_augmentation"]["N_rotations"]
        # index = np.random.randint(0,4)

        # probs = [0.5] + [0.5 / (n_rots - 1 )] * (n_rots - 1)
        if self.config["rotation_infos"]["ignore_symmetry_for_sampling"] == True:
            probs = [1 / (n_rots)] * n_rots

        elif self.config["rotation_infos"]["ignore_symmetry_for_sampling"] == False:
            if sym == '__SYM_NONE':
                probs = [1 / (n_rots)] * n_rots
            elif sym == '__SYM_ROTATE_UP_2':
                n_correct = int(n_rots/2) + 1
                probs = [1/n_correct] + int(((n_correct - 1)/2)) * [1/n_correct] + (n_rots - n_correct) * [0] +  int((n_correct - 1)/2) * [1/n_correct]
            elif sym == '__SYM_ROTATE_UP_4':
                if n_rots == 4:
                    n_correct = 1
                else:
                    n_correct = int(n_rots/4) + 1
                probs = [1/n_correct]+ int((n_correct - 1)/2) * [1/n_correct] + (n_rots - n_correct) * [0] +  int((n_correct - 1)/2) * [1/n_correct]
            else:
                probs = [1] + [0] * (n_rots -1)
    ...
    def get_R_and_T_and_S(self,augmented_bbox,R_gt,T_gt,f,img_size,model_path,gt_scaling_orig,category,sample_for_classification=False,sampled_S=None,sampled_T=None,sampled_R=None):
        ...
        R,offset_r,r_index = self.get_R_for_regression(R_gt,sampled_R,sym)
        ...
        S,s_correct,offset_s,S_normalised,bbox_3d = get_S(self.config,self.scaling_limits,self.scaling_medians,self.model_to_infos,model_path,gt_scaling_orig,category,sampled_S,self.kind,decrease_range_factor)
        correct = True * np.array([1]).astype(np.float32)
        r_correct = True



        return R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,bbox_3d,r_index
