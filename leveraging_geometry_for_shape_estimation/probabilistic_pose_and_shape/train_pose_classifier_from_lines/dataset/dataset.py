...
def get_S(config,scaling_limits,scaling_medians,model_to_infos,model_path,gt_scaling_orig,category,sampled_S,kind):
    ...
    if sampled_S != None:
        ...
        S = np.array(sampled_S)
        # S_normalised = S * bbox_3d
        ...
        if config['data']["sample_S"]["use_gt"] == True:
            if kind == 'train' and config["data_augmentation"]["augment_S"] == True:


                assert False, "this does not work with normalised scale and absolute scale"
                relative_offset = sample_relative_offset()
                S = gt_scaling_orig / (relative_offset + 1)
                # S_normalised = S * bbox_3d
                S_normalised = np.zeros(3)
                return S,True,relative_offset,S_normalised,bbox_3d

            else:
                return gt_scaling_orig,True,np.zeros(3),np.zeros(3),bbox_3d
        else:


            if kind == 'train':
                S = sample_S(gt_scaling,scaling_limits,config['data']["sample_S"])
            else:
            #    S = np.array(scaling_medians[model_path.split('/')[2]]) * bbox_3d

                # initialise with object mode
                # S = np.array(scaling_medians[model_path.split('/')[2]])

                # initialise with category mode
            ...
            # print('gt S')
            # relative_offset = relative_offset * 0
            # S = gt_scaling_orig

            # assert np.all(relative_offset < 10), (relative_offset,gt_scaling_orig,S,bbox_3d,model_path)
            # s_correct = np.mean(np.abs(relative_offset)) < 0.2
            s_correct = np.all(np.abs(relative_offset) < 0.2)
            return S,s_correct,relative_offset,S_normalised,bbox_3d
    ...
    # if config['data']["sample_S"]["use_gt"] == True:
    #     return gt_scaling_orig,True,torch.zeros(3),gt_scaling
    # else:
    #     if sampled_S == None:
    #         S_normalised = sample_S(gt_scaling,scaling_limits,config['data']["sample_S"])
    #         S = S_normalised / bbox_3d
    #     else:
    #         S = sampled_S

    #     relative_offset = gt_scaling_orig / S - 1
    #     # s_correct = np.mean(np.abs(relative_offset)) < 0.2
    #     s_correct = np.all(np.abs(relative_offset) < 0.2)
    #     return S,s_correct,relative_offset,S_normalised
...
def sample_S(gt_scaling,scaling_limits,sampling_info):
    ...
    if rand_n < sampling_info['percent_small']:
        S = sample_relative(gt_scaling,-sampling_info['limit_small'],sampling_info['limit_small'])
    elif rand_n > sampling_info['percent_small'] and rand_n < sampling_info['percent_small'] + sampling_info['percent_large']:
        S = sample_relative(gt_scaling,-sampling_info['limit_large'],sampling_info['limit_large'])
    else: 
...
def get_T(f,img_size,T_gt,config,bbox,threshold_correct,half_width_sampling_cube,kind,sampled_T=None):
    ...
    pose_sampling_params = config["data"]["pose_and_shape_probabilistic"]["pose"]
    gt_z = T_gt[2]
    sample_infos = config["data"]["sample_T"]

    xs,ys,zs = get_T_limits(f,img_size,sensor_width,pose_sampling_params,bbox,gt_z)

    rand_number = np.random.rand()

    if rand_number < sample_infos['percent_small']:
        # offset = sample_offset_threshold_cube(threshold_correct)
        offset = sample_offset_threshold_cube(threshold_correct/1.7)
    elif rand_number > sample_infos['percent_small'] and rand_number < sample_infos['percent_small'] + sample_infos['percent_large']:
        offset = sample_offset_around_threshold_cube(threshold_correct)
        # offset = sample_offset_large_cube(half_width_sampling_cube)
    else:
        # offset = sample_offset_random_from_limits(xs,ys,zs,gt_infos_detection["trans_mat"])
        offset = sample_offset_reprojected_in_bbox(bbox,img_size,sensor_width,f,T_gt,kind)

    if sample_infos["use_gt"] == True:
        offset = offset * 0
...
class Dataset_lines(data.Dataset):
    def __init__(self,config,kind,use_all_images=False):
        ...
        if config["data"]["what_models"] == "lines":
            ...
            # self.lines_3d = self.load_numpy_dir_compressed(config["data"]["dir_path_3d_points_and_normals"]+ 'points_and_normals',max_n=100000)
            # self.lines_3d = np.load(config["data"]["dir_path_3d_points_and_normals"]+ 'points_and_normals_combined.npz')
            print('LOAD NORMALISED POINTS, RESCALED')
            # self.lines_3d = torch.load(config["data"]["dir_path_3d_points_and_normals"]+ 'points_and_normals_combined.pt')
        ...
        if kind == 'train':
            ...
            if config['data']['use_canny'] == False:
                self.lines_2d = load_numpy_dir(self.dir_path_2d + '/lines_2d_filtered',max_n=config["training"]['max_number'] * 10)
                # self.lines_2d = load_numpy_dir(self.dir_path_2d + '/lines_2d_filtered_only_object_center',max_n=config["training"]['max_number'])
            elif config['data']['use_canny'] == True:    
                self.lines_2d = load_numpy_dir(self.dir_path_2d + '/canny_points_480_360_n_points_5000',max_n=config["training"]['max_number'])
            ...
            if self.use_all_images == True:
                ...
                self.dir_path_2d = config["data"][dir_key]
            # self.roca_all_preds = load_json('/scratches/octopus/fml35/datasets/own_datasets/leveraging_geometry_for_shape_estimation/scannet/data_03/val_roca/all_detection_infos.json')
        ...
        self.scaling_medians = load_json('/scratches/octopus/fml35/datasets/own_datasets/leveraging_geometry_for_shape_estimation/scannet/scaling/median_scale_by_model_id_category_median_if_not_in_train_normalised_scale.json')
        ...
        self.model_to_infos = get_model_to_infos_scannet()
        self.scaling_limits = load_json(self.dir_path_2d + '/global_stats/scaling_limits_increased_display_z.json')
        # self.scaling_medians = load_json('/scratches/octopus/fml35/datasets/own_datasets/leveraging_geometry_for_shape_estimation/scannet/scaling/median_scale_by_model_id_category_median_if_not_in_train.json')
    ...
    def get_gt_infos_by_detection_predicted(self):
        ...
        for detection_name in sorted(self.lines_2d):
            ...
            segmentation_info = load_json(self.dir_path_2d + '/segmentation_infos/' + detection_name.replace('.npy','.json'))
            detection_number = bbox_info["index_gt_objects"]

            gt_infos = self.gt_infos[gt_name]

            infos_img = {"img":gt_infos["img"],"img_size":gt_infos["img_size"],"focal_length":gt_infos["focal_length"]}

            all_infos[detection_name] = {**infos_img,**gt_infos['objects'][detection_number]}
            all_infos[detection_name]['bbox'] = segmentation_info['predictions']['bbox']
        return all_infos
    # def get_index_to_names(self):
    #     index_to_names = {}
    #     counter = 0
    #     for detection_name in sorted(self.lines_2d):
    ...
    #         model = self.gt_infos[gt_name]['objects'][detection_number]['model']
    #         model_3d_name = model.split('/')[1] + '_' + model.split('/')[2] + '.npy'

    #         if self.lines_3d[model_3d_name].shape[0] != 0 and self.lines_2d[detection_name].shape[0] != 0:
    ...
    def load_image_dir(self,dir,max_n=10000000,type='image'):
        ...
        #     # loaded = cv2.imread(dir + '/' + file,flag)
        #     loaded = all_files[file]
        #     if type == 'depth':
    ...
    def get_R(self,R_gt,sampled_R):
        if sampled_R == None:
            # this basically checks if train on R or not, for training on R sample from wider distribution
            if self.config["data"]["sample_R"]["use_gt"] == True:
                # if do not train on R still perform small augmentation
                if self.kind == 'val' or self.kind == 'val_roca':
                    R = R_gt
                    offset_R = np.array([0,0,0,1])
                
                else:
                    random_angles = np.random.uniform(-np.array(self.config["data_augmentation"]["change_R_angle_degree_old"]),self.config["data_augmentation"]["change_R_angle_degree_old"],size=3)
                    
                    # inv_offset_R = scipy_rot.from_euler('zyx',random_angles, degrees=True)
                    # R = np.matmul(R,inv_offset_R.as_matrix())
                    # offset_R = inv_offset_R.inv().as_quat()

                    # inefficient but just so dont make mistake
                    # actually intended augmentation above, but this is what had before
                    offset_R = scipy_rot.from_euler('zyx',random_angles, degrees=True).as_matrix()
                    R = np.matmul(offset_R,R_gt)
                    offset_R = scipy_rot.from_euler('zyx',random_angles, degrees=True).as_quat()
    ...
    def get_R_and_T_and_S(self,augmented_bbox,R_gt,T_gt,f,img_size,model_path,gt_scaling_orig,category,just_classifier,sampled_S=None,sampled_T=None,sampled_R=None):
        ...
        R,offset_r = self.get_R(R_gt,sampled_R)
        ...
        while in_loop == True:
            ...
            S,s_correct,offset_s,S_normalised,bbox_3d = get_S(self.config,self.scaling_limits,self.scaling_medians,self.model_to_infos,model_path,gt_scaling_orig,category,sampled_S,self.kind)

            if self.config["data"]["use_border_cases"] == True or self.kind == 'val' or self.kind == 'val_roca':
                in_loop = False
            
            t_edge = np.linalg.norm(offset_t) > self.config["data"]["range_border_cases_t"][0] and np.linalg.norm(offset_t) < self.config["data"]["range_border_cases_t"][1]
            s_edge = np.linalg.norm(offset_s) > self.config["data"]["range_border_cases_s"][0] and np.linalg.norm(offset_s) < self.config["data"]["range_border_cases_s"][1]
            combined_edge = t_edge or s_edge
            if not combined_edge:
                in_loop = False


        correct = r_correct and t_correct and s_correct
        correct = correct * np.array([1]).astype(np.float32)

        correct = np.array([1]).astype(np.float32)
        assert np.all((np.abs(offset_s) < 100)),  (S,s_correct,offset_s,S_normalised,bbox_3d)
...
if __name__ == '__main__':
    ...
    dataset = Dataset_lines(config,'train')

    out_path = '/scratch2/fml35/experiments/classify_T_from_lines/'
    for i in range(3):
