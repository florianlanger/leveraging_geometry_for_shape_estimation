...
def get_S(config,scaling_limits,scaling_medians,model_to_infos,model_path,gt_scaling_orig,category,sampled_S,kind):
    ...
    if kind == 'train':
        ...
        scaling_limits_low = scaling_limits['min'] + scaling_limits_range * 0.3
        scaling_limits_high = scaling_limits['max'] - scaling_limits_range * 0.3
...
class Dataset_lines(data.Dataset):
    def __init__(self,config,kind,use_all_images=False):
        ...
        self.transform_Rs = [scipy_rot.from_euler('zyx',[0,0,0], degrees=True).as_matrix(),
    ...
    def rotate_R_v2(self,R,sym):
        ...
        index = int(np.random.choice(4, 1, p=[0.4, 0.2, 0.2, 0.2]))
    ...
    def get_R_and_T_and_S(self,augmented_bbox,R_gt,T_gt,f,img_size,model_path,gt_scaling_orig,category,sampled_S=None,sampled_T=None,sampled_R=None):
        ...
        S,s_correct,relative_offset,S_normalised,bbox_3d = get_S(self.config,self.scaling_limits,self.scaling_medians,self.model_to_infos,model_path,gt_scaling_orig,category,sampled_S,self.kind)
