...
print('import dataset v4 ')
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset_v4_classify_rotation_and_predict_r_offset import load_numpy_dir,get_gt_infos,get_gt_infos_by_detection,bbox_to_lines,reproject_3d_lines,check_config,compute_lines_resized_torch,Dataset_lines,compute_lines_resized
...
class Dataset_points(Dataset_lines):
    ...
    def __init__(self,config,kind,use_all_images=False):
        ...
        self.rot_matrices_torch = self.load_compressed_file('/scratches/octopus_2/fml35/experiments/leveraging_geometry_for_shape/exp_117_scannet_models/models/rotations/R_T_torch.npz')

        # self.best_rotation_indices = load_json('/scratches/octopus/fml35/experiments/regress_T/evals/date_2022_07_02_time_13_27_29_EVAL_REFINE_1_date_2022_07_01_time_18_09_18_classification_adjusted_p_smaller_azim_network_epoch_000100_norm_medium_rotation_index_0/predictions/epoch_000050/translation_pred_scale_pred_rotation_init_for_classification_retrieval_roca_all_images_True/best_rotation_index.json')
        # self.best_rotation_indices = load_json('/scratches/octopus/fml35/experiments/regress_T/evals/date_2022_07_05_time_09_19_57_EVAL_REFINE_1_date_2022_07_03_time_12_15_58_classification_adjusted_p_smaller_azim_network_epoch_000200_norm_medium_rotation_index_0/predictions/epoch_000050/translation_pred_scale_pred_rotation_init_for_classification_retrieval_roca_all_images_True/best_rotation_index.json')
        self.best_rotation_indices = load_json('/scratches/octopus/fml35/experiments/regress_T/evals/date_2022_07_05_time_12_18_42_EVAL_REFINE_1_date_2022_07_01_time_18_09_18_classification_adjusted_p_smaller_azim_network_epoch_000100_norm_medium_rotation_index_0/predictions/epoch_000050/translation_pred_scale_pred_rotation_init_for_classification_retrieval_roca_all_images_True/best_rotation_index.json')
    ...
    def get_index_to_names_no_canny(self,kind):
        index_to_names = {}
        counter = 0
        for detection_name in sorted(self.lines_2d):
            gt_name = detection_name.rsplit('_',1)[0] + '.json'

            # if kind == 'train':
            #     detection_number = int(detection_name.split('_')[-1].split('.')[0])
            
            # elif kind == 'val':
            #     bbox_info = load_json(self.dir_path_2d + '/bbox_overlap/' + detection_name.replace('.npy','.json'))
            #     detection_number = bbox_info["index_gt_objects"]

            detection_number = int(detection_name.split('_')[-1].split('.')[0])

            for k in range(len(self.gt_infos[gt_name]['objects'])):
                if self.gt_infos[gt_name]['objects'][k]['index'] == detection_number:
                    model = self.gt_infos[gt_name]['objects'][k]['model']
                    break
            # print('load fake models')
            # model = 'model/bathtub/10caa8d340fa0fddfcd4910413c446d9/model_normalized.obj'
            # model_3d_name = model.split('/')[1] + '_' + model.split('/')[2] + '.npy'



            model_3d_name = model.split('/')[1] + '_' + model.split('/')[2] + '.npz'

            if self.lines_2d[detection_name].shape[0] != 0:
                index_to_names[counter] = [detection_name,gt_name,model_3d_name]
                counter += 1

        return index_to_names

    def get_index_to_names_no_canny_v2(self):
        index_to_names = {}
        counter = 0
        for gt_name in sorted(self.gt_infos):
            ...
            for i in range(len(self.gt_infos[gt_name]['objects'])):
                ...
                if np.any(np.array(self.scaling_medians[model.split('/')[2]]) < 0.01):
                    continue
    ...
    def get_index_to_names_roca_all_preds(self):
        index_to_names = {}
        counter = 0
        # print('DEBUG SMALL')
        # with open('/scratch/fml35/datasets/own_datasets/leveraging_geometry_for_shape_estimation/scannet/data_03/val_roca/classifier_roca_scale_detections.json','r') as f:
        #     classifier_preds = json.load(f)

        for gt_name in sorted(self.roca_all_preds):
            for i in range(len(self.roca_all_preds[gt_name])):
                # if self.roca_all_preds[gt_name][i]['associated_gt_infos']['matched_to_gt_object'] == True:
                if self.roca_all_preds[gt_name][i]['associated_gt_infos']['matched_to_gt_object'] == True: 
                    # if self.config['eval_roca']["what_rotation"] == 'lines':
                    if self.roca_all_preds[gt_name][i]["r_from_lines"] == None:
                        continue

                    # if self.roca_all_preds[gt_name][i]['detection'] not in classifier_preds:
                    #     continue
                    detection_name = self.roca_all_preds[gt_name][i]['detection'] + '.npy'

                    if self.config['data']["use_canny"] == False:
                        if detection_name not in self.lines_2d:
                            continue
                        if self.lines_2d[detection_name].shape[0] == 0:
                            continue

                    model_id_roca = self.roca_all_preds[gt_name][i]["scene_cad_id"][1]
                    model_id_gt = self.roca_all_preds[gt_name][i]["associated_gt_infos"]["model"].split('/')[2]
                    # if np.any(np.array(self.scaling_medians[model_id_roca]) < 0.01) or np.any(np.array(self.scaling_medians[model_id_gt]) < 0.01):
                    #     continue
    ...
    def get_index_to_names(self, kind):
        ...
        if kind == 'val_roca':
            ...
            elif self.use_all_images == False:
                return self.get_index_to_names_roca_all_preds()
    ...
    def create_info_RST_plus_modelid(self,R,S,T,model_name):
        info = torch.zeros((15,self.config["data"]["dims_per_pixel"]))
        # channel 3
        info[:,2] = info[:,2] * 0 + 5
        R = scipy_rot.from_matrix(R).as_quat()
    ...
    def create_info_RST_plus_modelid_plus_sym(self,R,S,T,model_name,sym):
        info = torch.zeros((16,self.config["data"]["dims_per_pixel"]))
        # channel 3
        info[:,2] = info[:,2] * 0 + 5

        if self.config["rotation_infos"]["random_rotate_R_before_input"] == True:
        ...
        info[15,:] = self.encode_symmetry(sym)
    ...
    def encode_symmetry(self,sym):
        encoded_sym = torch.zeros(self.config["data"]["dims_per_pixel"])
        encoded_sym[2] = 7
    ...
    def process_lines(self,detection_name):
        lines_2d = self.lines_2d[detection_name].to(self.device)
    ...
    def process_canny_points(self,gt_name):
        points_2d = self.lines_2d[gt_name].to(self.device)
        points_2d = torch.flip(points_2d,(1,))
    ...
    def get_infos_from_index(self,index,just_classifier):
        ...
        R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,bbox_3d,r_index = self.get_R_and_T_and_S(augmented_bbox,R_gt,T_gt,f,img_size,model_path,gt_scaling_orig,category,just_classifier)

        # history = self.initialise_history(R,T,S)
        history = None

        return detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index

    # def initialise_history(self,R,T,S):
    #     history_indices = torch.linspace(7,7+self.config["training"]["refinement_per_object"],self.config["training"]["refinement_per_object"]+1)
    #     history = torch.zeros((3 * (self.config["training"]["refinement_per_object"] + 1),self.config["data"]["dims_per_pixel"]))
    #     history[:,2] = torch.repeat_interleave(history_indices,3)

    #     R = scipy_rot.from_matrix(R).as_quat()
    #     history[0,3:7] = torch.tensor(R)
    #     history[1,3:6] = torch.tensor(T)
    #     history[2,3:6] = torch.tensor(S)

    #     # print('history',history)
    #     return history.numpy()

    def initialise_history(self,R,T,S):
        history_indices = torch.linspace(7,7+self.config["training"]["refinement_per_object"],self.config["training"]["refinement_per_object"]+1,device=self.device)
        history = torch.zeros((3 * (self.config["training"]["refinement_per_object"] + 1),self.config["data"]["dims_per_pixel"]),device=self.device)

        R = scipy_rot.from_matrix(R).as_quat()
        history[0,3:7] = torch.tensor(R).to(self.device)
        history[1,3:6] = torch.tensor(T).to(self.device)
        history[2,3:6] = torch.tensor(S).to(self.device)
    ...
    def get_infos_from_index_dict(self,index_dict):
        ...
        R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = index_dict['R'],index_dict['T'],index_dict['S'],index_dict['correct'],index_dict['offset_t'],index_dict['offset_s'],index_dict['offset_r'],index_dict['r_correct'],index_dict['sym'],index_dict['S_normalised'],index_dict['T_gt'],index_dict['R_gt'],gt_infos['img_size'],index_dict['S_gt'],use_roca_bbox,index_dict['bbox_3d'],index_dict['history'],index_dict['r_index']
    ...
    def get_infos_from_index_dict_roca(self,index_dict):
        ...
        R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = index_dict['R'],index_dict['T'],index_dict['S'],index_dict['correct'],index_dict['offset_t'],index_dict['offset_s'],index_dict['offset_r'],index_dict['r_correct'],index_dict['sym'],index_dict['S_normalised'],index_dict['T_gt'],index_dict['R_gt'],img_size,S_gt,use_roca_bbox,index_dict['bbox_3d'],index_dict['history'],index_dict['r_index']
    ...
    def get_infos_from_roca(self,index,just_classifier):
        ...
        R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,bbox_3d,r_index = self.get_R_and_T_and_S(augmented_bbox,R_gt,T_gt,f,img_size,model_path,S_gt,category,just_classifier,S,T,R)
    ...
    def __getitem__(self, tuple_index_dict_just_classifier):
        ...
        if type(index_dict) == dict:
            if self.kind == 'train' or self.kind == 'val':
                detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = self.get_infos_from_index_dict(index_dict)
            elif self.kind == 'val_roca':
                detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = self.get_infos_from_index_dict_roca(index_dict)
        else:
            index = index_dict
            if self.kind == 'train' or self.kind == 'val':
                ...
                if random_number < self.config["hard_example_mining"]["percentage_hard_examples"] and self.kind == 'train' and self.epoch > 0 and self.config["hard_example_mining"]["use"] == True:
                    ...
                    detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = self.get_infos_from_index(index,just_classifier)

            elif self.kind == 'val_roca':
                if self.use_all_images == False:
                    detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = self.get_infos_from_roca(index,just_classifier)
                elif self.use_all_images == True:
                    detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history,r_index = self.get_infos_from_roca_eval_without_gt(index,just_classifier)
        ...
        if self.config["data"]["input_RST_and_CAD_ID"] == True:
            ...
            info_RST = self.create_info_RST_plus_modelid_plus_sym(R,S,T,model_3d_name,sym).to(self.device)
            # info_RST = self.create_info_RST_plus_modelid(R,S,T,model_3d_name).to(self.device)
            list_info.append(info_RST)
        ...
        extra_infos = {'detection_name': detection_name,'gt_name': gt_name,'model_3d_name': model_3d_name, 'category': model_3d_name.split('_')[0],"offset_t":offset_t,"offset_s":offset_s,"offset_r":offset_r,"r_correct":r_correct,"sym":sym,"name_orientation":name_orientation,"roca_bbox":use_roca_bbox,"R":R,"T":T,"S":S,"correct":correct,"S_normalised":S_normalised,"img_size":img_size,'S_gt':S_gt,'T_gt':T_gt,'R_gt':R_gt,'bbox':bbox,'augmented_bbox':augmented_bbox,'bbox_3d':bbox_3d,"history":history,"r_index":r_index}
        

        assert offset_r[3] > -0.0001, offset_r[3]
        
        target = self.create_targets(correct,offset_t,offset_s,offset_r,r_index)
        ...
        # self.print_times()
        # assert torch.all(torch.abs(padded_info[2,:] - 5) > 0.1) , "padded_info[2] = {}".format(padded_info[2]) 

        # print("Loading times:",self.loading_times)
        # print("Loading times:",np.mean(self.loading_times[0]),np.mean(self.loading_times[1]),np.mean(self.loading_times[2]),np.mean(self.loading_times[3]),np.mean(self.loading_times[4]),np.mean(self.loading_times[5]),np.mean(self.loading_times[6]),np.mean(self.loading_times[7]),np.mean(self.loading_times[8]))
        # print("Loading times:",np.median(self.loading_times[0]),np.median(self.loading_times[1]),np.median(self.loading_times[2]),np.median(self.loading_times[3]),np.median(self.loading_times[4]),np.median(self.loading_times[5]),np.median(self.loading_times[6]),np.median(self.loading_times[7]),np.median(self.loading_times[8]))
        return padded_info,target,extra_infos

    def create_targets(self,correct,offset_t,offset_s,offset_r,r_index):
        if self.config['model']['regress_offsets'] == True:
            one_hot_rotation_classification = torch.zeros((self.config['data_augmentation']["N_rotations"]))
            one_hot_rotation_classification[r_index] = 1
            target = torch.cat([torch.Tensor(correct),torch.Tensor(offset_t),torch.Tensor(offset_s),torch.Tensor(offset_r),one_hot_rotation_classification],dim=0)
        else:
    ...
    def get_elev_and_azim_from_R(self,R):
        all_elevs = torch.Tensor([0.0,15.0,30.0,45.0])
        all_azims = torch.Tensor([0.0,22.5,45.0,67.5,90.0,112.5,135.0,157.5,180.0,202.5,225.0,247.5,270.0,292.5,315.0,337.5,360.0])

        t11 = time.time()
        angles = scipy_rot.from_matrix(R).as_euler('zyx',degrees=True)
        t12 = time.time()
        azim = 180 - angles[1]
        elev = - angles[2]

        
        closest_azim_index = torch.argmin(torch.abs(all_azims - azim))
        closest_elev_index = torch.argmin(torch.abs(all_elevs - elev))

        azim = all_azims[closest_azim_index]
        azim = torch.fmod(azim,360).item()
        elev = all_elevs[closest_elev_index].item()
        t13 = time.time()
        self.add_times('t12-t11',t12-t11)
        self.add_times('t13-t12',t13-t12)
        return elev,azim


    def get_elev_and_azim_from_R_v2(self,R):
        
        R_repeated = torch.unsqueeze(R,0).repeat(64,1,1)
        R_retrieval = self.rot_matrices_torch['R'].to(self.device)
        R_retrieval = torch.inverse(R_retrieval)
        angles = get_angle(R_repeated,R_retrieval)

        index = torch.argmin(angles)
        # elev = float(index // 16) * 15
        elev = float(torch.div(index, 16, rounding_mode='floor')) * 15
        azim = float(index % 16) * 22.5
        return elev,azim    

    def get_visibility_3d_points(self,model_3d_name,R,n_points):
        elev,azim = self.get_elev_and_azim_from_R_v2(R)
        name = 'elev_{}_azim_{}'.format(str(int(elev)).zfill(3),str(azim))
        index = int((elev / 15 ) * 16 + int(np.round(azim / 22.5)))
    

        if self.config['data']["mark_visible_points"] == True:
            visibility = self.visibility_3d_points[model_3d_name.split('.')[0]][index,:]
        elif self.config['data']["mark_visible_points"] == False:
            visibility = torch.ones(n_points).bool()
        t3 = time.time()
        visibility = visibility.to(self.device)
        # visibility = self.visibility_3d_points['bathtub_10caa8d340fa0fddfcd4910413c446d9.npz'][name]
    ...
    def create_embedding_3d(self,model_3d_name,R,T,S,img_size,K):
        ...
        t2 = time.time()
        # visibility,name_orientation = self.get_visibility_3d_points(model_3d_name,R,points.shape[0])
        if self.config['data']["mark_visible_points"] == True:
...
if __name__ == '__main__':
    ...
    dataset = Dataset_points(config,'train')
