...
# from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset import load_numpy_dir,get_gt_infos,get_gt_infos_by_detection,bbox_to_lines,reproject_3d_lines,check_config,compute_lines_resized_torch,Dataset_lines,compute_lines_resized
print('import dataset_v2 for classification')
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset_v2_classify_rotation import load_numpy_dir,get_gt_infos,get_gt_infos_by_detection,bbox_to_lines,reproject_3d_lines,check_config,compute_lines_resized_torch,Dataset_lines,compute_lines_resized
...
class Dataset_points(Dataset_lines):
    ...
    def __init__(self,config,kind,use_all_images=False):
        ...
        if self.config['data']["use_preloaded_depth_and_normals"] == True:
            ...
            self.norm_2d = self.load_image_dir(self.dir_path_2d + '/norm_{}_{}'.format(img_size_preloaded[0],img_size_preloaded[1]),max_n=4*config["training"]['max_number'],type='normal')
            ...
            self.norm_2d_dir = self.dir_path_2d + '/norm_{}_{}/'.format(img_size[0],img_size[1])
    ...
    def get_infos_from_index(self,index):
        ...
        R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,bbox_3d = self.get_R_and_T_and_S(augmented_bbox,R_gt,T_gt,f,img_size,model_path,gt_scaling_orig,category)
    ...
    def get_infos_from_roca(self,index):
        ...
        R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,bbox_3d = self.get_R_and_T_and_S(augmented_bbox,R_gt,T_gt,f,img_size,model_path,S_gt,category,S,T,R)
    ...
    def get_infos_from_roca_eval_without_gt(self,index):
    ...
    def get_rotation_modelpath_S_T(self,infos):
    ...
    def __getitem__(self, index_dict):
        ...
        if type(index_dict) == dict:
            ...
            if self.kind == 'train' or self.kind == 'val':
                ...
                if random_number < self.config["hard_example_mining"]["percentage_hard_examples"] and self.kind == 'train' and self.epoch > 0 and self.config["hard_example_mining"]["use"] == True:
                    ...
                    detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history = self.get_infos_from_index(index)

            elif self.kind == 'val_roca':
                if self.use_all_images == False:
                    detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history = self.get_infos_from_roca(index)
                elif self.use_all_images == True:
                    detection_name,gt_name,model_3d_name,K,R,T,S,correct,offset_t,offset_s,offset_r,r_correct,sym,S_normalised,T_gt,R_gt,bbox,augmented_bbox,img_size,S_gt,use_roca_bbox,bbox_3d,history = self.get_infos_from_roca_eval_without_gt(index)
        ...
        if self.config['data']['use_canny'] == False:
            ...
            if self.config['data']['use_all_points_2d'] == False:
                ...
                if self.config['data']["add_random_points"] == True:
        ...
        extra_infos = {'detection_name': detection_name,'gt_name': gt_name,'model_3d_name': model_3d_name, 'category': model_3d_name.split('_')[0],"offset_t":offset_t,"offset_s":offset_s,"offset_r":offset_r,"r_correct":r_correct,"sym":sym,"name_orientation":name_orientation,"roca_bbox":use_roca_bbox,"R":R,"T":T,"S":S,"correct":correct,"S_normalised":S_normalised,"img_size":img_size,'S_gt':S_gt,'T_gt':T_gt,'R_gt':R_gt,'bbox':bbox,'augmented_bbox':augmented_bbox,'bbox_3d':bbox_3d,"history":history}
    ...
    def create_embedding_3d(self,model_3d_name,R,T,S,img_size,K):
        ...
        if self.config['data']["mark_visible_points"] == True:
            assert False, "for debugging dont use"
