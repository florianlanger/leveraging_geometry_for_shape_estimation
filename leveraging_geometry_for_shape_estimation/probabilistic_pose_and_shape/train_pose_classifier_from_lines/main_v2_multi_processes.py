...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.batch_sampler import get_index_infos
...
def get_from_dataset(dataset,index):
    return dataset[index]


def get_batch_from_dataset(dataset,indices):

    # batch = asyncio.run(get_simple_batch_from_dataset(dataset,indices))

    loop = asyncio.get_event_loop()
    batch = loop.run_until_complete(get_simple_batch_from_dataset(dataset,indices))
    # print(len(batch))
    # print(batch[0].shape)
...
async def get_simple_batch_from_dataset(dataset,indices):
    ...
    # for i in indices:
    #     tasks.append(get_from_dataset(dataset, i))
    # print('done tasks')
    # batch = tasks
    # print(batch[0])
    # pool_end = time.time()
    ...
    with ProcessPoolExecutor(max_workers=1) as pool:
        for i in indices:
            tasks.append(loop.run_in_executor(pool, functools.partial(get_from_dataset, dataset, i)))
        print('done tasks')
        batch = await asyncio.gather(*tasks)

        # batch = pool.map(get_from_dataset, dataset, indices)
        pool_end = time.time()
        print(f'Process pool took {pool_end-pool_start}')
    ...
    # print(f'Serial took {serial_end-serial_start}')
...
def one_epoch_classifier(dataset,net,optimizer,criterion,epoch,writer,kind,device,config,exp_path):
    ...
    for num_batch in tqdm(range(N_total_batches)):
        ...
        for iter_refinement in range(N_refinements):
            t1 = time.time()
            data = get_batch_from_dataset(dataset,index_infos)
...
def evaluate_checkpoint(checkpoint_path,n_refinements,run_on_octopus=True,gpu_number=0):
    
    name_exp_eval = 'date_2022_' + checkpoint_path.split('/date_2022_')[1].split('/')[0] + '_' + checkpoint_path.split('/')[-1].split('.')[0] + '_depth_mini'
    ...
    config["general"]["output_dir"] = '/scratch/fml35/experiments/regress_T/evals'
    config["general"]["run_on_octopus"] = run_on_octopus
    config["general"]["gpu"] = gpu_number
    ...
    exp_path = '{}/{}_EVAL_REFINE_{}_{}'.format(config["general"]["output_dir"],datetime.now().strftime("date_%Y_%m_%d_time_%H_%M_%S"),n_refinements,name_exp_eval)
    print(exp_path)
...
def validate_roca(val_roca_dataset,network,optimizer,criterion,epoch,writer,device,config,exp_path):
    ...
    if epoch % config["training"]["save_interval"] == 0 and epoch != 0:
        ...
        for combo in all_combinations:
            ...
            if combo[2] == 'roca_init' or combo[2] == 'no_init':
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        network,optimizer,exp_path,config,device,start_epoch = evaluate_checkpoint(sys.argv[1],sys.argv[2],run_on_octopus=True,gpu_number=0)
    ...
    # checkpoint_path = '/scratch/fml35/experiments/regress_T/runs_10_T_and_R_and_S/date_2022_06_16_time_17_46_25_five_refinements_depth_aug_points_in_bbox_3d_coords/saved_models/network_epoch_000100.pth'
