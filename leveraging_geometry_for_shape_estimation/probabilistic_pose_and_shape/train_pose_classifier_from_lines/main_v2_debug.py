...
def one_epoch_classifier(dataset,net,optimizer,criterion,epoch,writer,kind,device,config,exp_path):
    ...
    len_dataset = 20000000
    print('debug')
    all_indices = create_shuffled_indices(len_dataset,config,kind)
    ...
    for num_batch in tqdm(range(N_total_batches)):
        ...
        for iter_refinement in range(N_refinements):
            ...
            # inputs = torch.zeros((config['training']["batch_size"],10000,10),device=device)
            ...
            # print('debug')
            # outputs = torch.ones((config['training']["batch_size"],11),device=device,requires_grad=True)
            outputs = net(inputs)
...
def create_data_loaders(config):
    ...
    if config["data"]["type"] == 'points' and config["data"]["what_models"] == "lines":
        ...
        from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.dataset.dataset_points_v2_debug import Dataset_points
    ...
    if config["data"]["type"] == 'points':
        if config['data']['targets'] == 'labels':
            ...
            train_dataset = Dataset_points(config,kind='val')
