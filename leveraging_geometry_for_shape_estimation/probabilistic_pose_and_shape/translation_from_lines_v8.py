...
from leveraging_geometry_for_shape_estimation.probabilistic_pose_and_shape.train_pose_classifier_from_lines.main import load_network, process_config
...
from probabilistic_formulation.factors.factors_T.factor_pose_classifier_v2 import get_factor_reproject_lines_multiple_T_classifier_v6,get_factor_reproject_lines_multiple_T_classifier_points_v6
...
def signal_bbox_or_line(global_config,model_path,target_folder,detection,device,R,Ts,bbox,f,w,h,sw,Ss,B,enforce_same_length,top_n_for_translation,line_dir_2d,scaling_lines,line_indices_2d=None,line_indices_3d=None,use_gt_retrieval=False,use_gt_R=False,network=None,classifier_config=None):
    ...
    if global_config["pose_and_shape_probabilistic"]["reproject_lines"]["signal"] == 'bbox':
        ...
        if classifier_config['data']['type'] == 'lines':
            ...
            factor_output_v4 = get_factor_reproject_lines_multiple_T_classifier_points_v6(torch.Tensor(R).to(device),torch.Tensor(Ts).to(device),torch.Tensor(Ss).to(device),torch.from_numpy(lines_3D).to(device),lines_2D.to(device),gt_name,B.to(device),f,sw,bbox,network,classifier_config,bs=30)
...
def get_pose_single_example(global_config,name,model_to_infos,pose_information,scale_range,stage_index,line_indices_2d=None,line_indices_3d=None,use_gt_retrieval=False,use_gt_R=False,specific_Ts=None,network=None,classifier_config=None):
...
def get_pose_for_folder(global_config):
    ...
    # for name in tqdm(sorted(os.listdir('/scratch2/fml35/experiments/leveraging_geometry_for_shape/exp_210_line_min_overlap/poses_R'))):
        ...
        for j in range(global_config["hierachial_grid_search"]["number_stages"]):
            ...
            pose_information,factor_information,factor_output = get_pose_single_example(global_config,name,model_to_infos,pose_information,scale_range,j,use_gt_R=use_gt_R,network=network,classifier_config=classifier_config)
