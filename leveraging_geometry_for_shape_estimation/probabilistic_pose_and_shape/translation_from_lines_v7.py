...
from probabilistic_formulation.factors.factors_T.factor_pose_classifier_v2 import get_factor_reproject_lines_multiple_T_classifier_v6
...
def signal_bbox_or_line(global_config,model_path,target_folder,detection,device,R,Ts,bbox,f,w,h,sw,Ss,B,enforce_same_length,top_n_for_translation,line_dir_2d,line_indices_2d=None,line_indices_3d=None,use_gt_retrieval=False,use_gt_R=False,network=None,classifier_config=None):
    ...
    if global_config["pose_and_shape_probabilistic"]["reproject_lines"]["signal"] == 'bbox':
        ...
        factor_output_v4 = get_factor_reproject_lines_multiple_T_classifier_v6(torch.Tensor(R).to(device),torch.Tensor(Ts).to(device),torch.Tensor(Ss).to(device),torch.from_numpy(lines_3D).to(device),lines_2D.to(device),gt_name,B.to(device),f,sw,bbox,network,classifier_config,bs=200)
...
def get_S(gt_scaling,gt=True):
    ...
    Ss = init_Ts((0.5,3,5),(0.5,3,5),(0.5,3,5))
...
def get_pose_single_example(global_config,name,model_to_infos,line_indices_2d=None,line_indices_3d=None,use_gt_retrieval=False,use_gt_R=False,specific_Ts=None,network=None,classifier_config=None):
    ...
    S = Ss[best_S_index]
    n_indices = 1
    pose_information = create_pose_info_dict(np.array(R),T,n_indices,max_factor,gt_pose_in_limits,gt_rot,gt_trans,best_R_possible,best_T_possible,xs,ys,zs,tilts,elevs,azims,gt_scaling,S,lines_3D_available,lines_2D_available)
