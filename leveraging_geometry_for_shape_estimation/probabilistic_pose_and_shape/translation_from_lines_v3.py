...
from probabilistic_formulation.factors.factors_T.factors_lines_multiple_T import get_factor_reproject_lines_multiple_T, get_factor_reproject_lines_multiple_T_threshold, get_factor_reproject_lines_multiple_T_with_Scale,get_factor_reproject_lines_multiple_T_threshold_map_single_3d_line
...
def signal_bbox_or_line(global_config,model_path,target_folder,detection,device,R,Ts,bbox,f,w,h,sw,Ss,B,enforce_same_length,top_n_for_translation,line_dir_2d,line_indices=None):
    ...
    if len(lines_2D.shape) == 1 or lines_3D.shape[0] == 0:
        ...
        if global_config["pose_and_shape_probabilistic"]["reproject_lines"]["signal"] == 'bbox':
            ...
            factors_batch,_,_ = get_factor_reproject_lines_multiple_T_threshold_map_single_3d_line(torch.Tensor(R).to(device),torch.Tensor(Ts).to(device),torch.from_numpy(lines_3D).to(device),lines_2D.to(device),B.to(device),f,sw,area_threshold,angle_threshold,only_allow_single_mapping_to_3d)
    ...
    return best_T_index,best_S_index,max_factor,lines_3D,lines_2D,lines_available,multiplier_lines,factors,area_threshold,angle_threshold,only_allow_single_mapping_to_3d
...
def get_Ts(f,w,h,sensor_width,global_config,bbox,gt_z,model_path,model_to_infos,gt_scaling,segmentation_infos,R):
