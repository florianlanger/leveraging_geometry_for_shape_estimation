...
def visualise_lines(original_lines,filtered_lines,bbox,img,vis_path):
...
def main():
    ...
    for name in tqdm(os.listdir(target_folder + '/segmentation_infos')):
        ...
        filtered_lines = filter_lines(lines,segmentation_infos["predictions"]["bbox"],threshold)

        np.save(output_path_lines,filtered_lines)

        if visualise:
            visualise_lines(lines,filtered_lines,segmentation_infos["predictions"]["bbox"],img,vis_path)
