...
from constants import BENCHMARK_CLASSES, CAD_TAXONOMY
...
class CategoryManager:
    def __init__(self, category_json, dataset_name):
        ...
        self._index_to_id = {
            v: k
            for k, v in zip(range(len(self._categories)), range(len(self._categories)))
            # metadata.thing_dataset_id_to_contiguous_id.items()
        }
        self._id_to_name = self._categories  # metadata.thing_classes
