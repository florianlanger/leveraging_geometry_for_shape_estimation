...
def collect_all_predictions(target_folder,global_config):
    collected_predictions = []

    print('need to do change in here if want multiple correct predictions per image')
    print('collect predictions')
    dir_bbox = determine_base_dir(global_config,'segmentation') + '/bbox_overlap'

    for name in tqdm(os.listdir(dir_bbox)):

        with open(dir_bbox + '/' + name.split('.')[0] + '.json','r') as f:
            bbox_overlap = json.load(f)

        if bbox_overlap['valid']:
            # if bbox_overlap['index_gt_objects'] == None:
            #     continue

            segmentation = load_json(determine_base_dir(global_config,'segmentation') + '/segmentation_infos/' + name.split('.')[0] + '.json')

            # with open(target_folder + '/segmentation_infos/' + name.split('.')[0] + '.json','r') as f:
            #     segmentation = json.load(f)

            gt_infos = load_json(determine_base_dir(global_config,'segmentation') + '/gt_infos/' + name.rsplit('_',1)[0] + '.json')
            # with open(target_folder + '/gt_infos/' + name.rsplit('_',1)[0] + '.json','r') as f:
            #     gt_infos = json.load(f)

            single_crop = {}
            single_crop["gt_cat"] = gt_infos["objects"][bbox_overlap['index_gt_objects']]["category"]
            ...
            if bbox_overlap['correct_category']:
                ...
                # if not os.path.exists(target_folder + '/selected_nn/' + name):
                #     break
...
def write_aps(global_config):
    ...
    categories_counter  = get_categories_counter(determine_base_dir(global_config,'segmentation') + '/gt_infos',categories)

    all_predictions = collect_all_predictions(target_folder,global_config)
