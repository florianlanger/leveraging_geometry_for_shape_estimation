...
from leveraging_geometry_for_shape_estimation.eval.F1_score.aps_new import compute_ap
...
def evaluate(projectdir, filename_cad_appearance, filename_annotations):
    ...
    binary_detections = {}
    scores = {}
    for cat in catid2catname:
        binary_detections[cat] = []
        scores[cat] = []
        ...
        for alignment in alignments: # <- multiple alignments of same object in scene
            ...
            for idx, model_gt in enumerate(groundtruth[key]):
                ...
                if is_same_class: # <-- proceed only if candidate-model and gt-model are in same class
                    ...
                    binary_detections[catid_cad].append(1*is_valid_transformation)
                    scores[catid_cad].append(float(alignment[-1]))
    ...
    for key in binary_detections:
        cat = catid2catname[key]
        print(benchmark_per_class[cat]["n_total"])
        ap_value = compute_ap(torch.Tensor(scores[key]),torch.Tensor(binary_detections[key]),benchmark_per_class[cat]["n_total"])
        print(cat,ap_value)
...
if __name__ == "__main__":
    scan2cad_dir = '/scratch2/fml35/datasets/scannet/scan2cad_annotations/'
    projectdir = '/scratch2/fml35/results/ROCA/results_per_scene/'
    evaluate(projectdir, scan2cad_dir +  "cad_appearances.json", scan2cad_dir + "full_annotations.json")