...
import quaternion
from leveraging_geometry_for_shape_estimation.eval.scannet import SE3,CSVHelper
import sys
...
def transform_single_raw(per_frame,scan2cad_full_annotations,output_path,dir_path_images):
    ...
    for image in tqdm(per_frame):
        ...
        for detection in per_frame[image]:
            ...
            scene = image.split('/')[0]
            frame = image.split('/')[2].split('.')[0]

            frame_4by4_path = dir_path_images + scene + '/pose/' + frame + '.txt'
...
def transform_single_raw_invert_own_no_invert_roca(per_frame,scan2cad_full_annotations,output_path,dir_path_images,not_invert_previous_predictions=True):
    print('Transform single raw')
...
if __name__ == '__main__':
    ...
    with open(target_folder + '/global_stats/eval_scannet/single_frame.json','r') as f:
        per_frame = json.load(f)

    with open(global_config["dataset"]["dir_path_images"] + '../scan2cad_annotations/full_annotations.json') as json_file:
        scan2cad_full_annotations = json.load(json_file)

    dir_path_images = global_config["dataset"]["dir_path_images"]
    output_path = target_folder + '/global_stats/eval_scannet/raw_results.csv'


    transform_single_raw(per_frame,scan2cad_full_annotations,output_path,dir_path_images)