...
def evaluate(projectdir, filename_cad_appearance, filename_annotations):
    ...
    for file0 in glob.glob(projectdir + "/*.csv"):
        ...
        total_number_predictions += len(alignments)
        ...
        for alignment in alignments: # <- multiple alignments of same object in scene
            ...
            # -> read from .csv file
            catid_cad = catid_cad = str(alignment[0]).zfill(8)
            id_cad = alignment[1]
    ...
    print('TOTAL Number predictions: ',total_number_predictions)
    return instance_mean_accuracy, class_mean_accuracy


if __name__ == "__main__":
    scan2cad_dir = '/scratch2/fml35/datasets/scannet/scan2cad_annotations/'
    projectdir = '/scratch2/fml35/results/ROCA/results_per_scene_filtered/'
