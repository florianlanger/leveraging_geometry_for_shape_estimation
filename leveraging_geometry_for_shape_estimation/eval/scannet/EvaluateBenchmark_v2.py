...
def calc_rotation_diff_considering_symmetry(q,q_gt,sym):
    ...
    for force_same_retrieval in [False,True]:
        ...
        for file0 in glob.glob(projectdir + "/*.csv"):
            ...
            for alignment in alignments: # <- multiple alignments of same object in scene
                ...
                # if np.abs(float(alignment[12])-0.92133074) < 0.00001:
                #     print('appearances_cad[id_scan]',appearances_cad[id_scan])
                #     print('n_appearances_allowed',n_appearances_allowed)
                #     print('appearance_counter[cadkey]',appearance_counter[cadkey])

                if appearance_counter[catid_cad] >= n_appearances_allowed:
                    continue

                # if catid_cad == "02933112":
                #     print('appearances_cad[id_scan]',appearances_cad[id_scan])
                #     print('cad key',cadkey)
                #     print('n_appearances_allowed',n_appearances_allowed)
                ...
                for idx, model_gt in enumerate(groundtruth[key]):
                    ...
                    if is_same_class: # <-- proceed only if candidate-model and gt-model are in same class
                        ...
                        # sym = cad2info[(model_gt["catid_cad"], model_gt['id_cad'])]["sym"]
                        ...
                        error_rotation = calc_rotation_diff_considering_symmetry(q,q_gt,sym)
...
if __name__ == "__main__":
    ...
    results_file = global_config["general"]["target_folder"] + '/global_stats/results_scannet_new_filter_changed_s_and_sym_4.txt'
