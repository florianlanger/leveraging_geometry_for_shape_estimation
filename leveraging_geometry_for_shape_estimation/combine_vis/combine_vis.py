...
def get_pose_for_folder(global_config):
    ...
    for name in tqdm(os.listdir(target_folder + '/metrics')):
        ...
        if gt_infos["img"] in visualisation_list:
            ...
            for i in range(top_n_retrieval):
                    ...
                    seg = load_image(target_folder + '/segmentation_vis/' + detection)
                    matches = load_image(target_folder + '/matches_vis/' + retrieval,(256,512))

                    matches_quality = load_image(target_folder + '/matches_quality_vis/' + retrieval)
                    keypoints = load_image(target_folder + '/keypoints_vis/' + detection) #.replace('.png','.' + file_ending))
                    lines = load_image(target_folder + '/lines_2d_filtered_vis/' + detection)
                    factors_lines = load_image(target_folder + '/factors_lines_vis/' + orientation)
                    lines_T = load_image(target_folder + '/T_lines_vis/' + orientation)
