    ...
    for i in tqdm(range(faces.shape[0])):
        for pair in [[0,1],[1,2],[2,0]]:
            ...
            if indices not in checked_pairs and indices[::-1] not in checked_pairs:
                ...
                # print(face_normals[faces_id_shared[0]])
                # print(face_normals[faces_id_shared[1]])
                # print(face_normals[faces_id_shared[2]])
                # print(face_normals[faces_id_shared[3]])
                # print('---------')
                time_1 = time()
                ...
                if len(faces_id_shared) > 2:
                    ...
                    n_2 = face_normals[faces_id_shared[0]]
                    for i in range(1,len(faces_id_shared)):
                        if (torch.abs(face_normals[faces_id_shared[i]] - n_1) > 0.0001).any() and (torch.abs(face_normals[faces_id_shared[i]] + n_1) > 0.0001).any():
                            n_2 = face_normals[faces_id_shared[i]]
                            break
                    #     print(face_normals[faces_id_shared[i]])
                    # print(len(faces_id_shared))
                    # continue

                # elif len(faces_id_shared) == 4:
                #     n_1 = face_normals[faces_id_shared[0]]
                #     n_2 = face_normals[faces_id_shared[2]]
...
def main():
    ...
    dir_existing_lines = '/scratches/octopus_2/fml35/experiments/leveraging_geometry_for_shape/exp_117_scannet_models/models/lines/'


    with open(target_folder + "/model_list.json",'r') as f:
        model_list = json.load(f)["models"]

    target_folder = target_folder + '/lines_exp02'
    ...
    #     if not 'bed_' in model_list[j]["name"]:
    ...
    for j in tqdm(range(0,len(model_list))):
        ...
        if not 'table_5c7dde9' in model_list[j]["name"]:
            continue
        print(dir_existing_lines + model_list[j]["category"] + '_' + model_list[j]["model"].split('/')[2] + '.npy')
        if os.path.exists(dir_existing_lines + model_list[j]["category"] + '_' + model_list[j]["model"].split('/')[2] + '.npy'):
            continue
        ...
        vertex_id_to_face_id = -1 * torch.ones((verts_torch.shape[0],1000),dtype=int)
