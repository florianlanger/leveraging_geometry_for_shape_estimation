...
def get_index_ranges(converter,cad_appearances,category):

    indices = []
    for cad_model in cad_appearances:
        cad_model_category = cat_id_to_cat(cad_model.split('_')[0])
        if cad_model_category == category: 
            name = cad_model_category + '_' + cad_model.split('_')[1]
            start_index, stop_index = converter.name_to_index_range(name)
            indices += list(range(start_index,stop_index))
    return indices


def find_nearest_neighbours_wild(real_embedding,syn_embeddings,category,number_nearest_neighbours,converter):
...
def find_nearest_neighbours_scene(real_embedding,syn_embeddings,category,number_nearest_neighbours,converter,cad_appearances):


    indices = get_index_ranges(converter,cad_appearances,category)
    relevant_model_embeddings = syn_embeddings[indices,:]
    distances = euclidean_dist(relevant_model_embeddings,real_embedding)
    ...
    nn_dict["nearest_neighbours"] = []
    for j in range(number_nn):
        model_index = indices[nearest_neighbours[j].item()]
        nn_info = converter.index_to_info_dict(model_index)
        nn_info["path"] = '{}/{}/elev_{}_azim_{}.png'.format(nn_info["category"],nn_info["name"].replace(nn_info["category"] + '_',''),nn_info["elev"],nn_info["azim"])
        nn_info["model_index"] = model_index
        nn_info["distance"] = float(sorted_distances[j].item())
        nn_dict["nearest_neighbours"].append(nn_info)
...
if __name__ == "__main__":   
    ...
    if global_config["retrieval"]["gt"] == "False":
        ...
        with open(global_config["dataset"]["dir_path_images"] + '/../scan2cad_annotations/cad_appearances.json','r') as f:
        ...
        with torch.no_grad():
            ...
            for name in tqdm(sorted(os.listdir(target_folder + '/cropped_and_masked_small'))):
                ...
                if global_config["retrieval"]["style"] == 'wild':
                    nn = find_nearest_neighbours_wild(real_embedding,syn_embeddings,predicted_category,number_nn,converter)
                elif global_config["retrieval"]["style"] == 'scene':
                    nn = find_nearest_neighbours_scene(real_embedding,syn_embeddings,predicted_category,number_nn,converter,appearance_dict[name.split('-')[0]])
