...
@torch.no_grad()
def compare_meshes(
    pred_meshes, gt_meshes, meshes_path,visualisations, num_samples=10000, scale="gt-10", thresholds=None, reduce=True, eps=1e-8
):
    """
    ...
    if visualisations:
        save_ply(meshes_path + '/rescaled_pred.ply',pred_points[0].cpu())
        save_ply(meshes_path + '/rescaled_gt.ply',gt_points[0].cpu())
    ...
    if torch.is_tensor(pred_points) and torch.is_tensor(gt_points):
        ...
        metrics = _compute_sampling_metrics(
            pred_points, pred_normals, gt_points, gt_normals, thresholds, eps, meshes_path, visualisations
        )
    else:
        ...
        for cur_points_pred, cur_points_gt in zip(pred_points, gt_points):
            cur_metrics = _compute_sampling_metrics(
                cur_points_pred[None], None, cur_points_gt[None], None, thresholds, eps, meshes_path, visualisations
            )
...
def _compute_sampling_metrics(pred_points, pred_normals, gt_points, gt_normals, thresholds, eps, meshes_path, visualisations):
    """
    ...
    # save which points are within threshold
    ...
    modified_Hausdorf = (pred_to_gt_dists2.sum(dim=1) + gt_to_pred_dists2.sum(dim=1)) / (pred_to_gt_dists2.shape[1] + gt_to_pred_dists2.shape[1])
    metrics["Chamfer-L2"] = chamfer_l2
    metrics["modified_Hausdorf"] = modified_Hausdorf
    ...
    for t in thresholds:
        ...
        if visualisations:
            np.save(meshes_path + '/predicted_correct_threshold_{}.npy'.format(t),(pred_to_gt_dists < t).cpu().numpy())
            np.save(meshes_path + '/gt_correct_threshold_{}.npy'.format(t),(gt_to_pred_dists < t).cpu().numpy())
