...
def create_setup_calibration_matrix(scene,frame,dir_path,device,n_mesh):
    ...
    # focal_length = 2*K[0,0]/width
    ppoint = torch.Tensor([[width/2-K[0,2],height/2 - K[1,2]]])/ (width/2) # torch.Tensor([width/2,height/2])
    # ppoint = torch.Tensor([[width/2-K[0,2],height/2 - K[1,2]]])/ torch.Tensor([width/2,height/2])
    ppoint = -ppoint
    # print('focat length: ', focal_length)
    # print('ppoint: ', ppoint)
    # extract information from calibration matrix
    ...
    K = torch.Tensor([[[fx,0,ppoint[0,0],0],[0,fy,ppoint[0,1],0],[0,0,0,1],[0,0,1,0]]])
    # print(K)
    K[0,2:4,2:4] = torch.Tensor([[0,1],[1,0]])
...
def main():
    ...
    output_main_dir = '/scratches/octopus_2/fml35/datasets/scannet/scannet_frames_25k_own_data_exp_12_calibration_matrix/'
